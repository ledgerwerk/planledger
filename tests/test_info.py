...
from planledger.models import AppContext
from planledger.storage import collect_inventory, load_workspace
...
def test_info_default_human_lists_plans_and_footprint(
    ...
    # Fresh init + plan create stays at schema 2; info reports it as-is (read-only).
...
def test_info_json_envelope_shape(initialized_workspace: Path, invoke_json) -> None:
    ...
    for key in (
        ...
        assert key in r["storage"], key
    # Fresh init + plan create stays at schema 2 (only workshop commands migrate to 3).
    assert r["storage"]["schema_version"] == 2
    assert r["storage"]["active_plan_id"] == "plan-0001"
...
def test_collect_inventory_is_readonly_and_does_not_migrate_schema(
    ...
    storage_path = initialized_workspace / ".planledger" / "storage.yaml"
    before = yaml.safe_load(storage_path.read_text())
    assert before["schema_version"] == 2  # fresh init writes schema_version 2

    workspace = load_workspace(
        AppContext(root=None, cwd=initialized_workspace, json_output=False)
    )
    inventory = collect_inventory(workspace)

    # Reported as-is, not migrated to 3 by the read-only inventory walk.
    assert inventory["storage"]["schema_version"] == 2
    assert inventory["plan_count"] == 0
    assert inventory["workshop_count"] == 0

    after = yaml.safe_load(storage_path.read_text())
    assert after["schema_version"] == 2  # unchanged
